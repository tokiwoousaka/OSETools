...
namespace Alice.Extensions
{
    public static class EnumerableExtensions
    {
        ...
        public static TResult Switch<T, TSwitcher, TResult>(this T source, Func<T, TSwitcher, bool> judge, params Case<TSwitcher, TResult>[] cases)
        {
            return cases.Where(x => judge(source, x.From)).First().Result;
        }
    }

    public static class Case
    {
        public static Case<T1, T2> New<T1, T2>(T1 from, T2 result)
        {
            return new Case<T1, T2>() { From = from, Result = result };
        }
    }
    ...
    public static class StringExtension
    {
        ...
        public static IEnumerable<string> ParseIndent(this string source, string indentStart, string separator, string indentEnd)
        {
            var buffer = "";
            var indent = 0;
            var seek = false;
            ...
            foreach (var c in source)
            {
                ...
                if (seek)
                {
                    if (buffer.EndsWith(indentStart))
                        indent++;
                    if (buffer.EndsWith(indentEnd))
                    {
                        indent--;
                        if (indent == 0)
                            seek = false;
                ...
                else if (buffer.EndsWith(indentStart))
                {
                    indent++;
                    seek = true;
                }

                else if (buffer.EndsWith(separator))
                {
                    buffer = buffer.Remove(buffer.Length - separator.Length);
                    yield return buffer;
                    buffer = "";
                }
